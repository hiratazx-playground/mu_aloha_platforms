...
def makeAndroidImage(outputbin_dir, output_dir, root_dir, device_name, dtb_name) :
    ...
    dtb_path = os.path.join(root_dir, "Platforms", "SurfaceDuo1Pkg", "Device", device_name, 'DeviceTreeBlob', 'Android', 'android-' + dtb_name)

    logging.info("Generating bootpayload.bin")

    '''
      Payload:
        | BootShim.bin | SM8150_EFI.fd |
    '''
...
## woa-msmnile patch start
        ...
        self.env.SetValue("ACTIVE_PLATFORM", f"{PackageName}/{PlatformName}{'NoSb' if not SecureBoot else ''}.dsc", "Platform Hardcoded")
## woa-msmnile patch end
